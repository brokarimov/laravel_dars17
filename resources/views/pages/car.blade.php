        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    ...
                    @endif
                    @if (auth()->user()->hasPermission('cars.create'))
                                ...
                                <tbody id="">
                                    @foreach ($models as $model)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    @if (auth()->user()->hasPermission('cars.show'))
                                                        <a href="/cars/{{$model->id}}" class="btn btn-primary mx-2">Show</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('cars.edit'))
                                                        <a href="/cars/{{$model->id}}/edit"
                                                            class="btn btn-warning mx-2">Update</a>
                                                    @endif
                                                    @if (auth()->user()->hasPermission('cars.destroy'))
                                                        <form action="/cars/{{$model->id}}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
